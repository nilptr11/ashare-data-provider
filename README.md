...
维护层面向长期基础库，不做 30/60/120 日滚动删除；30/60/120 日只是验收或分析读取窗口。执行流程是：
...
检查最近 30 个交易日和最近 30 个自然日事件分区的 mart 完整性。30 日用于日常维护验收；策略分析需要 60/120 日或更长窗口时，在 `analysis bundle` 中显式传入：
...
各 mart 数据集的含义、分区键、维护口径和验收方式见 [基础数据维护对照表](references/maintenance-dataset-catalog.md)；bundle、context、summary 和行业证据 prompt 的读取方式见 [分析能力对照表](references/analysis-capability-catalog.md)。
...
ashare maintain report --end-date 2026-06-24 --profile full --trade-days 30 --event-days 7
...
ashare maintain report --end-date 2026-06-24 --profile full --trade-days 30 --event-days 30
```
...
面向 LLM 或分析框架时，不直接读零散接口缓存，而是从 mart 生成 bundle。`--trade-days` 是策略读取窗口，下面以 120 个交易日为例；短线框架可以传 30/60，趋势或财务验证可以传更长窗口：
...
LLM 使用本项目数据时，优先读已经生成的结构化产物，而不是现场拼零散接口。全市场分析优先使用 `ashare analysis bundle` 输出；个股研究优先使用 `research-context` 和 `research-summary` 输出；数据产物与行业证据 prompt 的对应关系参考 [分析能力对照表](references/analysis-capability-catalog.md)；判断 Tushare 权限、替代源和外部证据边界时参考 `src/ashare_data_provider/source_policy.json`。
...
`research-context` 会按内置 `source_policy.json` 跳过无权限 Tushare 接口，尽力采集 LLM 可读取的单股结构化上下文。单个接口失败不会中断输出，而是写入 `data_gaps`，方便上层 prompt 明确标注缺口。默认 `--profile basic` 只采集核心行情、估值、指数和行业日频数据；需要财务、事件、资金流时再使用 `--profile standard` 或 `--profile full`。
